...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $action = $_POST['action'] ?? '';
    $id = (int)($_POST['id'] ?? 0);
    // SE ELIMINÓ strtoupper: Ahora guarda el texto tal cual se ingresa
    $nombre = trim($_POST['nombre'] ?? '');

    try {
        if ($action === 'nuevo') {
            $pdo->prepare("INSERT INTO hospitales (nombre) VALUES (?)")->execute([$nombre]);
        } else if ($action === 'editar') {
            $pdo->prepare("UPDATE hospitales SET nombre = ? WHERE id = ?")->execute([$nombre, $id]);
        } else if ($action === 'eliminar') {
            $pdo->prepare("DELETE FROM hospitales WHERE id = ?")->execute([$id]);
        }
...
$sql_count = "SELECT COUNT(*) FROM hospitales" . $where;
...
/* ===== CARGA DE DATOS ===== */
$sql_items = "SELECT * FROM hospitales $where ORDER BY nombre ASC LIMIT $por_pagina OFFSET $offset";
...
<div class="container mb-5">
    <div class="main-card">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-center mb-4">
            <h4 class="text-primary font-weight-bold mb-3 mb-md-0">Catálogo de Hospitales</h4>
            
            <div class="search-box">
                <form method="GET" class="input-group">
                    <input type="text" name="q" class="form-control" placeholder="Buscar hospital..." value="<?php echo e($busqueda); ?>">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="text-center">
                            ...
                            <form method="POST" class="d-inline" onsubmit="return confirm('¿Eliminar este hospital?')">
    ...
    <div class="modal-dialog">
        <form method="POST" class="modal-content">
            <div class="modal-header bg-light">
                <h5 class="modal-title font-weight-bold" id="mTitle">Nuevo Hospital</h5>
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    <label class="small font-weight-bold">Nombre del Hospital:</label>
...
<script>
    function abrirModal(d=null){
        ...
        $('#mTitle').text(d?'Editar Hospital':'Nuevo Hospital');
        $('#id').val(d?d.id:'');
