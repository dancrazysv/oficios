...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $action = $_POST['action'] ?? '';
    $id = (int)($_POST['id'] ?? 0);
    // Eliminado strtoupper: ahora guarda tal cual se escribe
    $nombre = trim($_POST['nombre'] ?? '');

    try {
        if ($action === 'nuevo') {
            $pdo->prepare("INSERT INTO departamentos (nombre) VALUES (?)")->execute([$nombre]);
        } else if ($action === 'editar') {
            $pdo->prepare("UPDATE departamentos SET nombre = ? WHERE id = ?")->execute([$nombre, $id]);
...
$sql_count = "SELECT COUNT(*) FROM departamentos" . $where;
...
/* ===== CARGA DE DATOS ===== */
$sql_items = "SELECT * FROM departamentos $where ORDER BY nombre ASC LIMIT $por_pagina OFFSET $offset";
...
<div class="container mb-5">
    <div class="main-card">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-center mb-4">
            <h4 class="text-primary font-weight-bold mb-3 mb-md-0">Catálogo de Departamentos</h4>
            
            <div class="search-box">
                <form method="GET" class="input-group">
                    <input type="text" name="q" class="form-control" placeholder="Buscar departamento..." value="<?php echo e($busqueda); ?>">
...
<div class="modal fade" id="modalD" tabindex="-1">
    <div class="modal-dialog">
        <form method="POST" class="modal-content">
            <div class="modal-header bg-light"><h5 class="modal-title font-weight-bold" id="mTitle">Nuevo Departamento</h5></div>
            <div class="modal-body">
                <input type="hidden" name="action" id="action">
                <input type="hidden" name="id" id="id">
                <div class="form-group">
                    <label class="small font-weight-bold">Nombre:</label>
                    <input type="text" name="nombre" id="nombre" class="form-control" placeholder="Ej: San Salvador" required>
                </div>
            </div>
...
<script>
    function abrirModal(d=null){
        ...
        $('#mTitle').text(d?'Editar Departamento':'Nuevo Departamento');
        $('#id').val(d?d.id:'');
