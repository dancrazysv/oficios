...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $action = $_POST['action'] ?? '';
    // SE ELIMINÓ strtoupper: Ahora guarda tal cual se escribe
    $id = trim($_POST['id_manual'] ?? '');
    $nombre = trim($_POST['nombre'] ?? '');

    try {
        if ($action === 'nuevo') {
            $pdo->prepare("INSERT INTO tipos_constancia (id, nombre) VALUES (?, ?)")->execute([$id, $nombre]);
            $msg = "<div class='alert alert-success shadow-sm'>Tipo de constancia creado con éxito.</div>";
        } else if ($action === 'editar') {
            $pdo->prepare("UPDATE tipos_constancia SET nombre = ? WHERE id = ?")->execute([$nombre, $id]);
            $msg = "<div class='alert alert-success shadow-sm'>Registro actualizado.</div>";
        } else if ($action === 'eliminar') {
            $pdo->prepare("DELETE FROM tipos_constancia WHERE id = ?")->execute([$id]);
...
if ($busqueda !== '') {
    $where = " WHERE id LIKE ? OR nombre LIKE ? ";
...
$sql_count = "SELECT COUNT(*) FROM tipos_constancia" . $where;
...
/* ===== CARGA DE DATOS ===== */
$sql_items = "SELECT * FROM tipos_constancia $where ORDER BY id ASC LIMIT $por_pagina OFFSET $offset";
...
<head>
    <meta charset="UTF-8">
    <title>Admin - Tipos de Constancia</title>
...
<div class="container mb-5">
    <div class="main-card">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-center mb-4">
            <h4 class="text-primary font-weight-bold mb-3 mb-md-0">Tipos de Constancia</h4>
            
            <div class="search-box">
                <form method="GET" class="input-group">
                    <input type="text" name="q" class="form-control" placeholder="Buscar por código o nombre..." value="<?php echo e($busqueda); ?>">
        ...
        <div class="table-responsive">
            <table class="table table-hover">
                <thead class="bg-light">
                    <tr>
                        <th width="150">Código ID</th>
                        <th>Descripción de Constancia</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="text-muted font-weight-bold"><?php echo e($i['id']); ?></td>
                        ...
                        <td class="text-center">
                            ...
                            <form method="POST" class="d-inline" onsubmit="return confirm('¿Eliminar?')">
                                ...
                                <input type="hidden" name="id_manual" value="<?php echo e($i['id']); ?>">
    ...
    <div class="modal-dialog">
        <form method="POST" class="modal-content">
            <div class="modal-header bg-light">
                <h5 class="modal-title font-weight-bold" id="mTitle">Nuevo Tipo de Constancia</h5>
                <button type="button" class="close" data-dismiss="modal"><span>&times;</span></button>
            </div>
            <div class="modal-body">
                <input type="hidden" name="action" id="action">
                <div class="form-group">
                    <label class="small font-weight-bold">Código ID (Único):</label>
                    <input type="text" name="id_manual" id="id_manual" class="form-control" placeholder="Ej: const_nacimiento" required>
                </div>
                <div class="form-group">
                    <label class="small font-weight-bold">Descripción:</label>
                    <input type="text" name="nombre" id="nombre" class="form-control" placeholder="Nombre de la constancia" required>
                </div>
            </div>
...
<script>
    function abrirModal(d=null){
        ...
        $('#mTitle').text(d?'Editar Tipo de Constancia':'Nuevo Tipo de Constancia');
        $('#id_manual').val(d?d.id:'').prop('readonly', d?true:false);
