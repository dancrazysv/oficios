...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $nombre_val = trim($_POST['nombre_completo'] ?? '');
    $correo     = trim($_POST['correo_electronico'] ?? '');
    $tel        = trim($_POST['telefono'] ?? '');
    $tipo_doc   = (int)($_POST['tipo_documento_id'] ?? 0);
    $num_doc    = trim($_POST['numero_documento_limpio'] ?? '');

    try {
        if ($action === 'nuevo') {
            $stmt = $pdo->prepare("INSERT INTO solicitantes (nombre_completo, correo_electronico, telefono, tipo_documento_id, numero_documento_limpio) VALUES (?, ?, ?, ?, ?)");
            $stmt->execute([$nombre_val, $correo, $tel, $tipo_doc, $num_doc]);
            $msg = "<div class='alert alert-success shadow-sm'>Solicitante registrado con éxito.</div>";
        } else if ($action === 'editar') {
            $stmt = $pdo->prepare("UPDATE solicitantes SET nombre_completo = ?, correo_electronico = ?, telefono = ?, tipo_documento_id = ?, numero_documento_limpio = ? WHERE id = ?");
            $stmt->execute([$nombre_val, $correo, $tel, $tipo_doc, $num_doc, $id]);
            $msg = "<div class='alert alert-success shadow-sm'>Datos actualizados correctamente.</div>";
...
if ($busqueda !== '') {
    $where = " WHERE s.nombre_completo LIKE ? OR s.numero_documento_limpio LIKE ? ";
...
$sql_count = "SELECT COUNT(*) FROM solicitantes s" . $where;
...
/* ===== CARGA DE DATOS ===== */
$tipos_doc = $pdo->query("SELECT * FROM tipos_documento ORDER BY nombre ASC")->fetchAll();

$sql_items = "SELECT s.*, t.nombre as doc_txt 
              FROM solicitantes s 
              LEFT JOIN tipos_documento t ON s.tipo_documento_id = t.id 
              $where 
    ...
    <div class="collapse navbar-collapse" id="navbarNav">
        <ul class="navbar-nav mr-auto">
            ...
            <?php if (in_array($rol, ['administrador', 'supervisor'])): ?>
                ...
                <li class="nav-item"><a class="nav-link" href="../panel_envios.php">Envíos <span class="badge badge-danger"><?php echo $stats_envios['ERROR'] ?? 0; ?></span></a></li>
            <?php endif; ?>
        </ul>
...
<div class="container mb-5">
    <div class="main-card">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-center mb-4">
            <h4 class="text-primary font-weight-bold mb-3 mb-md-0">Gestión de Solicitantes</h4>
            <div class="search-box">
                <form method="GET" class="input-group">
                    <input type="text" name="q" class="form-control" placeholder="Buscar por nombre o documento..." value="<?php echo e($busqueda); ?>">
                    <div class="input-group-append"><button class="btn btn-primary" type="submit">Buscar</button></div>
                </form>
            </div>
            <button class="btn btn-success font-weight-bold" onclick="abrirModal()">+ NUEVO REGISTRO</button>
        </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="font-weight-bold"><?php echo e($i['nombre_completo']); ?></td>
                        <td><small class="badge badge-secondary"><?php echo e($i['doc_txt'] ?? 'N/A'); ?></small></td>
                        <td><code><?php echo e($i['numero_documento_limpio']); ?></code></td>
                        <td><small><?php echo e($i['correo_electronico']); ?><br><?php echo e($i['telefono']); ?></small></td>
                        <td class="text-center">
                            ...
                            <form method="POST" class="d-inline" onsubmit="return confirm('¿Eliminar?')">
                                <input type="hidden" name="action" value="eliminar"><input type="hidden" name="id" value="<?php echo $i['id']; ?>">
...
<div class="modal fade" id="modalS" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <form method="POST" class="modal-content">
            <div class="modal-header bg-light"><h5 class="modal-title font-weight-bold" id="mTitle">Nuevo Solicitante</h5></div>
            <div class="modal-body">
                ...
                <div class="row">
                    <div class="col-md-12 mb-3">
                        ...
                        <input type="text" name="nombre_completo" id="m_nombre" class="form-control" placeholder="Escriba el nombre completo" required>
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="small font-weight-bold">Correo Electrónico:</label>
                        <input type="email" name="correo_electronico" id="m_correo" class="form-control">
                    </div>
                    <div class="col-md-6 mb-3">
                        <label class="small font-weight-bold">Teléfono:</label>
                        <input type="text" name="telefono" id="m_tel" class="form-control">
                    </div>
                    <div class="col-md-6">
                        <label class="small font-weight-bold">Tipo Doc:</label>
                        <select name="tipo_documento_id" id="m_tipo" class="form-control">
                            <?php foreach($tipos_doc as $td): ?><option value="<?php echo $td['id']; ?>"><?php echo $td['nombre']; ?></option><?php endforeach; ?>
                        </select>
                    </div>
                    <div class="col-md-6">
                        <label class="small font-weight-bold">Número Documento:</label>
                        <input type="text" name="numero_documento_limpio" id="m_num" class="form-control" required>
                    </div>
                </div>
            </div>
            <div class="modal-footer"><button type="button" class="btn btn-secondary" data-dismiss="modal">Cerrar</button><button type="submit" class="btn btn-primary font-weight-bold">Guardar</button></div>
        </form>
    </div>
</div>
...
<script>
    function abrirModal(d=null){
        ...
        $('#mTitle').text(d?'Editar Solicitante':'Nuevo Solicitante');
        $('#id').val(d?d.id:'');
        $('#m_nombre').val(d?d.nombre_completo:'');
        $('#m_correo').val(d?d.correo_electronico:'');
        $('#m_tel').val(d?d.telefono:'');
        $('#m_tipo').val(d?d.tipo_documento_id:'');
        $('#m_num').val(d?d.numero_documento_limpio:'');
        $('#modalS').modal('show');
    }
</script>
</body>
