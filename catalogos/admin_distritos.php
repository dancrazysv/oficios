...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'nuevo') {
            $pdo->prepare("INSERT INTO distritos (nombre, municipio_id) VALUES (?, ?)")->execute([$nombre, $muni_id]);
        } else if ($action === 'editar') {
            $pdo->prepare("UPDATE distritos SET nombre = ?, municipio_id = ? WHERE id = ?")->execute([$nombre, $muni_id, $id]);
        } else if ($action === 'eliminar') {
            $pdo->prepare("DELETE FROM distritos WHERE id = ?")->execute([$id]);
        }
...
if ($busqueda !== '') {
    $where = " WHERE dist.nombre LIKE ? OR muni.nombre LIKE ? ";
...
$sql_count = "SELECT COUNT(*) FROM distritos dist JOIN municipios muni ON dist.municipio_id = muni.id" . $where;
...
/* ===== CARGA DE DATOS ===== */
$municipios = $pdo->query("SELECT m.id, m.nombre, d.nombre as depto FROM municipios m JOIN departamentos d ON m.departamento_id = d.id ORDER BY d.nombre, m.nombre")->fetchAll();

$sql_items = "SELECT dist.*, muni.nombre as muni_nombre 
              FROM distritos dist 
              JOIN municipios muni ON dist.municipio_id = muni.id 
              $where
              ORDER BY muni.nombre ASC, dist.nombre ASC 
...
<div class="container mb-5">
    <div class="main-card">
        <div class="d-flex flex-column flex-md-row justify-content-between align-items-center mb-4">
            <h4 class="text-primary font-weight-bold mb-3 mb-md-0">Gestión de Distritos</h4>
            <div class="search-box">
                <form method="GET" class="input-group">
                    <input type="text" name="q" class="form-control" placeholder="Buscar distrito o municipio..." value="<?php echo e($busqueda); ?>">
        ...
        <div class="table-responsive">
            <table class="table table-hover">
                ...
                <tbody>
                    ...
                    <tr>
                        <td class="small text-muted"><?php echo e($i['muni_nombre']); ?></td>
...
<div class="modal fade" id="modalDist" tabindex="-1">
    <div class="modal-dialog">
        <form method="POST" class="modal-content">
            <div class="modal-header bg-light">
                <h5 class="modal-title font-weight-bold" id="mTitle">Nuevo Distrito</h5>
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    <label class="small font-weight-bold">Municipio Perteneciente:</label>
                    <select name="municipio_id" id="muni_id" class="form-control" required>
                        <option value="">Seleccione municipio...</option>
                        <?php foreach($municipios as $m): ?>
                            <option value="<?php echo $m['id']; ?>"><?php echo "[".e($m['depto'])."] ".e($m['nombre']); ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>

                <div class="form-group">
                    <label class="small font-weight-bold">Nombre del Distrito:</label>
                    <input type="text" name="nombre" id="nombre" class="form-control" placeholder="Nombre del distrito" required>
                </div>
            </div>
...
<script>
    function abrirModal(d=null){
        ...
        $('#mTitle').text(d?'Editar Distrito':'Nuevo Distrito');
        $('#id').val(d?d.id:'');
        $('#nombre').val(d?d.nombre:'');
        $('#muni_id').val(d?d.municipio_id:'');
        $('#modalDist').modal('show');
    }
</script>
</body>
</html>