...
if ($filtro_tipo === 'OFICIO_INST' || $filtro_tipo === '') {
    $sql_i = "SELECT oi.id, oi.referencia_salida AS referencia, oi.fecha, i.nombre_institucion AS registrado_a, i.unidad_dependencia, i.ubicacion_sede, NULL AS municipio_destino_id, 0 AS enviado_correo, oi.estado_validacion, u.nombre_completo AS elaborado_por, 'OFICIO_INST' AS tipo_documento, oi.creado_por, oi.email_envio AS contacto_email, oi.ruta_pdf_final FROM oficios_institucionales oi LEFT JOIN instituciones i ON oi.id_institucion = i.id LEFT JOIN usuarios u ON oi.creado_por = u.id $sql_w_oi ORDER BY oi.fecha DESC";
...
$count_pendientes_envio = count(array_filter($docs_of, fn($d) =>
    ((int)($d['enviado_correo'] ?? 0)) === 0 && $d['estado_validacion'] === 'APROBADO'
));
...
.btn-preview{ background-color:#6f42c1; color:#fff; border:none; }
.btn-preview:hover{ background-color:#5a32a3; color:#fff; }
...
<div class="card stat-card bg-primary text-white shadow-sm"><div class="card-body"><h6 class="text-uppercase font-weight-bold">TOTAL FILTRADO</h6><h2><?php echo ($count_o + $count_c + $count_oi); ?></h2></div></div>
</div>
<div class="col-md-2 col-6 mb-2">
<div class="card stat-card bg-dark text-white shadow-sm"><div class="card-body"><h6 class="text-uppercase font-weight-bold">OFICIOS</h6><h2><?php echo $count_o; ?></h2></div></div>
</div>
<div class="col-md-2 col-6 mb-2">
<div class="card stat-card bg-info text-white shadow-sm"><div class="card-body"><h6 class="text-uppercase font-weight-bold">CERTIFICACIONES</h6><h2><?php echo $count_c; ?></h2></div></div>
</div>
<div class="col-md-2 col-6 mb-2">
<div class="card stat-card bg-success text-white shadow-sm"><div class="card-body"><h6 class="text-uppercase font-weight-bold">INSTITUCIONALES</h6><h2><?php echo $count_oi; ?></h2></div></div>
</div>
<div class="col-md-2 col-6 mb-2">
<div class="card stat-card bg-warning text-dark shadow-sm"><div class="card-body"><h6 class="text-uppercase font-weight-bold">PEND. VALIDAR</h6><h2><?php echo $count_p; ?></h2></div></div>
</div>
<div class="col-md-2 col-6 mb-2">
<a href="?envio_filtro=PENDIENTE" style="text-decoration:none">
<div class="card stat-card bg-danger text-white shadow-sm"><div class="card-body"><h6 class="text-uppercase font-weight-bold">PEND. ENVIO</h6><h2><?php echo $count_pendientes_envio; ?></h2></div></div>
</a>
</div>
</div>
...
<option value="<?php echo e($a); ?>" <?php echo ($filtro_area === $a) ? 'selected' : ''; ?>><?php echo e($a); ?></option>
<?php endforeach; ?>
</select>
</div>
<?php endif; ?>
...
<option value="<?php echo (int)$op['id']; ?>" <?php echo ($filtro_operador == $op['id']) ? 'selected' : ''; ?>>
...
$es_oficio = ($tipo_doc === 'OFICIO');
$es_const = ($tipo_doc === 'CONSTANCIA');
$es_oficio_inst = ($tipo_doc === 'OFICIO_INST');
$badge_color = $es_oficio ? 'dark' : ($es_const ? 'info' : 'success');
$aprobado = ($estado === 'APROBADO');
$pendiente = ($estado === 'PENDIENTE');
$puede_revisar = in_array($rol, ['administrador','supervisor'], true);
...
<td><small class="badge badge-<?php echo $badge_color; ?>"><?php echo $es_oficio_inst ? 'OFICIO INST.' : $tipo_doc; ?></small></td>
<td>
...
<td><span class="badge badge-<?php echo ($aprobado) ? 'success' : (($pendiente) ? 'warning' : 'danger'); ?>"><?php echo e($estado); ?></span></td>
<td>
...
<?php else: ?>
    ...
    <?php if ($pendiente): ?>
        <?php if ($es_oficio_inst): ?>
            <?php if ($rol === 'normal'): ?>
                <a href="preview_pdf.php?id=<?php echo $id; ?>&type=OFICIO_INST" target="_blank" class="btn btn-sm btn-preview">Vista Previa</a>
        ...
        <?php else: ?>
            <?php if ($puede_revisar): ?>
                <button class="btn btn-sm btn-primary btn-habilitar" data-id="<?php echo $id; ?>" data-type="<?php echo $tipo_doc; ?>">Habilitar</button>
                <a href="view_pdf.php?type=<?php echo $tipo_doc; ?>&id=<?php echo $id; ?>" target="_blank" class="btn btn-sm btn-outline-secondary">Revisar</a>
            <?php elseif ($rol === 'normal'): ?>
                <a href="preview_pdf.php?id=<?php echo $id; ?>&type=<?php echo $tipo_doc; ?>" target="_blank" class="btn btn-sm btn-preview">Vista Previa</a>
            <?php endif; ?>
        <?php endif; ?>
    <?php endif; ?>
    ...
    <?php if ($aprobado): ?>
        ...
        <?php elseif ($es_const): ?>
            <a href="<?php echo !empty($ruta_pdf) ? e($ruta_pdf) : 'reimprimir_constancia_pdf.php?id=' . $id; ?>" target="_blank" class="btn btn-sm btn-success">Ver/Imprimir</a>
        <?php elseif ($es_oficio_inst): ?>
            <a href="<?php echo !empty($ruta_pdf) ? e($ruta_pdf) : 'generar_pdf_institucional.php?ref=' . urlencode($doc['referencia']); ?>" target="_blank" class="btn btn-sm btn-success">Ver PDF</a>
...
$(function(){
    ...
    $('.btn-habilitar').click(function(){
        ...
        let url = 'habilitar_constancia.php';
        if (tipo === 'OFICIO') {
            url = 'habilitar_pdf.php';
        } else if (tipo === 'OFICIO_INST') {
            url = 'habilitar_oficio_inst.php';
        }
        $.post(url, {
            id: btn.data('id'),
            ...
            else { alert(r.message); btn.prop('disabled', false).text('Habilitar'); }
        }, 'json');
    });
